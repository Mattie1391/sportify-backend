...
//教練取得所有課程(可以限制特定一門課程)的每月觀看次數、總計觀看次數、總收益、月收益
async function getCoachAnalytics(req, res, next) {
  try {
    ...
    //建立所有觀看次數資料的粗查詢器，後面依據資料區劃各字做分組、排序、篩選、取得資料包的作業
    const allViewStats = await viewRepo
      ...
      .select([
        ...
        "DATE_TRUNC('month', v.date) AS period",
        "SUM(view_count) AS view_counts",
      ])
    ...
    //加總所有課程觀看次數
    const total_views = allViewStats.reduce((sum, row) => sum + parseInt(row.view_counts), 0);
